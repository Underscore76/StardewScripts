...
def getTravelingMerchantStock(gameID,dayNumber):
	def _invalid_idx(index):
		invalid = True if index in [158,159,160,161,162,163,326,341,413,437,439,454,460,645,681,682,688,689,690,774,775] else False
		return invalid
	...
	for i in range(10):
		...
		while True:
			...
			if _invalid_str(strArray):
				continue
			break
		stock[i] = [strArray[0],max(rand.Next(1,11)*100, int(strArray[1])*rand.Next(3,6)), 1 if (rand.Sample() > 0.1) else 5]
	return stock
...
if __name__ == '__main__':
	...
	for i in range(1,16*3+1):
		...
		for bundle in bundleList:
			...
			if items:
				itemList.append([[dayToYSD(day),a[0],a[1]] for a in getTravelingMerchantStock(gameID, day).values() if a[0] in items])
		for bundle in bundleList:
			day = 7*i
			items=checkDay(gameID,day,bundle)
			if items:
